...
/*                                                +#+#+#+#+#+   +#+           */
/*   Created: 2018/03/22 20:31:41 by obamzuro          #+#    #+#             */
/*   Updated: 2018/03/22 20:43:32 by obamzuro         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
static int		char_strchr_check(void)
{
	...
	{
		returns[0] = strchr(strcases[cases_iter], strcases[AMOUNT_TESTS][cases_iter]);
		returns[1] = ft_strchr(strcases[cases_iter], strcases[AMOUNT_TESTS][cases_iter]);
//		printf("returns[0] = %p\nreturns[1] = %p\n", returns[0], returns[1]);
...
//			cases[AMOUNT_TESTS], cases[cases[AMOUNT_TESTS]], strcases[AMOUNT_TESTS][cases[AMOUNT_TESTS]]);
		if (!comp_arr(returns[0], returns[1], strcases[cases_iter] + TEST_ARR_SIZE - returns[0] + 1) ||
			...
			printf("STRCHR CHAR cases_iter %d - OK\n", cases_iter);
	}
	return (1);
}
